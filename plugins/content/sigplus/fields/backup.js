...
window.addEvent('domready', function () {
	...
	*/
	function settings_backup() {
		var params = $$('*[name^=params]').filter(function (item) {
		...
		var data = params.map(function (item) {  // iterate name-value pairs
			var match = /^params\[(.*)\]$/.exec(item.get('name'));
			if (match) {
	...
	*/
	function settings_restore() {
		...
		$('extension-settings-list').get('value').split('\n').each(function (item) {
			...
			var params = $$('*[name^="params[' + param_name + ']"], *[name^="params[' + params_mapping[param_name] + ']"]');
		...
		var customparam = $pick($$('*[name="params[settings]"]'));
		if (customparam) {
